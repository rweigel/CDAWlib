  ...
  endelse
   ...
   if (no_data_avail eq 0) then begin
	 ...
	 erase ; erases background and makes it white 
      ...
      if keyword_set(mpeg) then begin
         if(reportflag eq 1) then printf,1,format='(a)','MPEG='+mpeg
         print,format='(a)','MPEG='+mpeg
      endif else begin ; if it's not mpeg then it's gif:
         if(reportflag eq 1) then printf,1,format='(a)','MGIF='+mgif
         print,format='(a)','MGIF='+mgif
      endelse

;$Author: ryurow $
;$Date: 2024/03/12 16:13:38 $
;$Header: /home/cdaweb/dev/control/RCS/timeaxis_text.pro,v 1.49 2024/03/12 16:13:38 ryurow Exp ryurow $
;$Locker: ryurow $
;$Revision: 1.49 $
;-------------------------------------------------------------
;+
...
pro timeaxis_text, t, jd=jd, form=form, nticks=nticks, $
	  ...
	  charsize=charsize , scale=scale, plabeloffset=plaboff, onlylabel = onlylabel,$
...
if keyword_set(hlp) then begin
   ...
   print,'   SCALE=s       Seperate scaleing factor for labels'
...
; Default value for label scaling factor
; R. Yurow (March 12, 2024)
if n_elements(scale) eq 0 then scale = 1.18 ; default label scaling factor is 1
...
; Created new procedure to time axis lengths to tick positions are aligned
; with interesting and useful time intervals (days, weeks, months, etc)
; Ron Yurow (March 25, 2020)
;tnaxes, xmn, xmx, nticks, tt1, tt2, dtt, t1, t2, dt, form=frm   ; Axis numbers.
time_interval, xmn, xmx, nticks, tt1, tt2, dtt, t1, t2, dt, form=frm   ; Axis numbers.
...
; This line uses the original values for the interval start and end.
; What we to do is use the values returned by time_interval
; Ron Yurow (March 25, 2020)
; going back to the original values for start and end; values returned
; from time_interval does not work on our regular plots.  It does 
; however work on inventory plots
; Jen Sun (Sep.29 2020)
if (n_elements(jd) ne 0) and ((xmx-xmn) gt 864000.) then begin
;if (n_elements(jd) ne 0) and ((tt2-tt1) gt 864000.) then begin
; Calculate jd1 and jd2 using the values returned by time_interval
; Ron Yurow (March 25, 2020)
; using second set of jd1 and jd2 causes issue with missing tickmarks
; Jen Sun (Sep. 29th 2020)
   jd1 = jd + xmn/86400.d0	; JD of xmn.
   jd2 = jd + xmx/86400.d0	; JD of xmx.
   ;jd1 = jd + tt1/86400.d0	; JD of xmn.
   ;jd2 = jd + tt2/86400.d0	; JD of xmx.
   ...
   ; RCJ 03/05/2018  Changed: 20 -> 10
   ; Don't limit the number of major ticks to 10.  Lets allow up to 14 ticks.  Also
   ; lets plan for the case where we get more double this number...
   ; Ron Yurow  (March 25, 2019)
   ; if n_elements(mjr) gt 10 then begin
   reduce_by = 1
   IF N_ELEMENTS (mjr) gt 14 THEN reduce_by = 2
   IF N_ELEMENTS (mjr) gt 28 THEN reduce_by = 3
   if reduce_by gt 1 then begin
       mjr1=0
      ; for i=0,n_elements(mjr)-1,2 do mjr1=[mjr1,mjr[i]]
      for i=0,n_elements(mjr)-1,reduce_by do mjr1=[mjr1,mjr[i]]
      mjr=mjr1[1:*]
   endif   
   if keyword_set(noyear) then frm = getwrd(frm,-99,-1,/last)
   ; The following two lines are commented out so that tick marks will be placed
   ; at fixed intervals.  Tick mark marks will generally ingnore month boundries.
   ; Ron Yurow (January 30, 2024)
...
;form='h$:m$:s$@y$ n$ d$'
; Use renamed version of TIME_LABEL.
; R. Yurow (Apr 26, 2023)
lab = spdf_time_label(v, form, jd=jd)
...
for i = 0, n_elements(v)-1 do begin	; Major (Labeled) ticks.
   ...
   if keyword_set(plaboff) then begin
	 ...
	 ;/dev, size=size, dy=dy, yspace=ysp, charthick=charthick
      xprint, /init, ix[i],iyt+plaboff, $
	 /dev, size=size*scale, dy=dy, yspace=ysp, charthick=charthick
   endif else begin

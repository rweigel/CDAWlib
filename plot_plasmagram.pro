;$Author: rcjohns1 $ 
;$Date: 2024/03/01 01:09:56 $
;$Header: /home/cdaweb/dev/control/RCS/plot_plasmagram.pro,v 1.153 2024/03/01 01:09:56 rcjohns1 Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.153 $
;+
...
;       COLORTAB = if not set (will be default rainbow), 
;                  2/13/2023 working on viritis and batlow options
;
; OUTPUTS:
...
FUNCTION plot_plasmagram, astruct, zname, $
		      ...
		      MOVIE=MOVIE, TOP_TITLE=TOP_TITLE,COLORTAB=COLORTAB
...
;TJK 10/20/2022 adjust this logic slightly because some values are
;very close to the validmin and doing above makes the values black on
;the colorbar.  We want the value more in the middle of the range.
       idmax = idmin+0.05          ; add so the colobar will be happy
       idmin = idmin-0.05          ; add so the colobar will be happy
...
; is set BEFORE we use it...
  ...
  if keyword_set(GIF) then begin
    ...
    deviceopen,6,fileOutput=GIF,sizeWindow=[xs+xco+axis_size,ys+40+axis_size],colortab=colortab
      if (no_data_avail eq 0) then begin
...
;       print,'IMAGE=',GIF
     ...
     deviceopen,7,fileOutput=PNG,sizeWindow=[xs+xco+axis_size,ys+40+axis_size],colortab=colortab
     if (no_data_avail eq 0) then begin

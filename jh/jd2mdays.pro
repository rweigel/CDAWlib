	...
	if dx le 30 then begin
	  ...
	  for i = 0, n_elements(mjds)-1 do begin
	    ...
	    if lo lt hi then begin
	      ...
	      ; RCJ 28Mar2018  In tests, not including that middle term
	      ;     to the array makes the labels better spaced
	      ;     when time range includes end of one month
	      ;     and beginning of the next.
	      if mjds[i] le majv[n_elements(majv)-1]+dx then $
	          majv = [majv, makei(lo,hi,dx)] else $
	          majv = [majv, mjds[i],makei(lo,hi,dx)]
	    endif else begin
	      majv = [majv, mjds[i]]
	    endelse

...
;TJK 2/1/2019 add code to look for Z variables scaletype in order to
;set linear or log scaling.
a = tagindex('SCALETYP',tag_names(astruct.(vnum)))
logZ = 0 ; set default to linear scaling
if(a[0] ne -1) then begin
	scaletyp = astruct.(vnum).SCALETYP
	if (scaletyp eq 'log') then logZ = 1 ; set log scaling
endif
...
;based on the actual latitude and longiture box for the data and then
...
;TJK 3/2/2021 get the fillvalue for latitude so we can exclude it from
...
; Assign longitude variable
  ;a = tagindex(strtrim(ipts[1],2),tag_names(astruct))
  if (strpos(strlowcase(ipts[0]),'lat')) ne -1 then $
   ; testing 'lat' again but the difference is the 2 lines below.
...
;TJK 3/1/2021 get the fillvalue for longitude so we can exclude it from
;min/max calculations                         
   a3 = tagindex('FILLVAL',tag_names(astruct.(a[0])))
   if (a3[0] ne -1) then begin & b=size(astruct.(a[0]).FILLVAL)
        ...
        if (not(walk2d_x) or not(walk2d_y)) then begin ; Record varying
         if (size(glat, /n_dimensions) eq 3) then begin
           glat = glat[*,*,start_frame:stop_frame]
                          ...
                          ;need to create more
;TJK 6/24/2019 get the type from the variable
;           gsize = size(glat)
...
;	print, '!d.n_colors = ',!d.n_colors
	...
	if(wc gt 0) then idmin = min(idat[above1],/nan) else idmin = zvmin ;TJK 4/8/2004 - add for log scaling
      endif
...
;TJK 1/31/2019 the data set name/directory string is too long
;to fit on one "IDL line", so have to use a formatted print to force
...
;
      ...
      if  (descriptor[0] eq 'VIS') then begin
          ...
          if  OLD_VIS then begin
              ...
              ENDIF ELSE BEGIN
                  glat [*, *, j] = rotate (glat [*, *, j], 5)
                  glon [*, *, j] = rotate (glon [*, *, j], 5)
...
;have the traditional 3 dimensions
           ...
           proj = strmid(project,0,3)
           imagedata = idat[*,*,j]
           goodata = where(imagedata ne 0, goodcnt)
           if (goodcnt gt 0) then begin
;           print, 'image data found, min and max',min(imagedata,max=imax) & print, imax  
;           print, 'image index = ',j
           borders = 1 ; set Continents default on
           map_linethick = 2; enabless continents and defines the line thickness
           ;TJK 1/24/2019 restructure code to allow another case for IBEX
           ...
           case proj of
             ...
             'HEX': begin ;Make special case for IBEX data sets
               longitude = glon[*,j]
               latitude = glat[*,j]
               nogrid = 0 ;want grids but no continents
               borders = 0 ; this does not disable continents
               map_linethick = 0; this does not disable continents either, see below
               auroral_image, imagedata, longitude, latitude, $
                              method=method, nogrid=nogrid, centerLonLat=CENTERLONLAT, proj=map_proj,$
                              /nocolorbar, position=position,fillValue=-1.0e+31,SYMSIZE=1.0,$
                              status=status,label=2, central_azimuth=central_azimuth, $
                              rangelonlat=thisrangelonlat,mapcolor=mapcolor,GLINETHICK=1.5
                                ;/TJK 1/31/2019 Don't want continents for
                                ;IBEX/HEX case and simply passing in
                                ;zero's for these keywords doesn't turn them off (boo)
                                ;MLINETHICK=map_linethick, continents=borders
          ...
          endcase
;TJK 3/2/2021 added a section for ICON to determine min/max of lat/long
;excluding the fill data and also smooth.
          ...
          if (n_elements(ionosphere_proj) ge 2) then begin
           ...
           if (ionosphere_proj[1] eq 'GOLD') then begin
             ;hardcode centerlonlat slightly northwest of center
             centerlonlat = [-60.0,10.0]
             if (smooth) then method = "PO" ;if we want smoothing
           endif
        endif

        if (proj ne 'HEX') then begin
...
; Print orientation
  ...
  if COLORBAR then begin
    ...
    xwindow = !x.window
;TJK 3/2/2020 - don't adjust the window for plotting the color bar
; we want the color bar to be the same height as the mapped image
; so commenting out these settings for x and y window (they originated
; in the thumbnail section of the plot_map_images code but
; aren't desired here.
;    !y.window[1]=!y.window[1]
;    !x.window[1]=0.858
;    !y.window=[0.1,0.9]
    offset = 0.01
;print, 'DEBUG movie y.window = ',!y.window
;print, 'DEBUG movie x.window = ',!x.window

  ;TJK 2/1/2019 allow linear AND log scaled colorbar

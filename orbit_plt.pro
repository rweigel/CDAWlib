;$Author: rcjohns1 $
;$Date: 2023/02/24 17:44:40 $
;$Header: /home/cdaweb/dev/control/RCS/orbit_plt.pro,v 1.176 2023/02/24 17:44:40 rcjohns1 Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.176 $
...
pro draw_orbit,epoch,x,y,z,kks,xmp,rhomp,xbs,rhobs,doymark,hrmark,hrtick, $
...
;print, 'xz region'
      ...
      plot_orbit,x,z,kks,region = region, color=color,lnthick=lnthick
...
;print, 'xy region'
        ...
        plot_orbit,x,y,kks,region = region, color=color,lnthick=lnthick
...
;print, 'yz region'
        ...
        plot_orbit,y,z,kks,region = region, color=color,lnthick=lnthick
...
;         if(z[j] lt 0) then r[j]=-1.D0*r[j]
        ...
        plot_orbit,x,r,kks,region = region, color=color,lnthick=lnthick
...
; RCJ 03Jan2022 This portion moved further down 
;numstr=astrc.(0)
...
if not keyword_set (xumn) then xumn=0
if not keyword_set (xumx) then xumx=0
if not keyword_set (yumn) then yumn=0
if not keyword_set (yumx) then yumx=0
if not keyword_set (zumn) then zumn=0
if not keyword_set (zumx) then zumx=0
if not keyword_set (rumn) then rumn=0
if not keyword_set (rumx) then rumx=0
; RCJ 24Feb2023  These values are being set in map_keywords.pro
;   The problem is that they could be both set to 0. If that's the case
;   use autoscale.
if (xumn eq 0 and xumx eq 0) then autoscl=1L
if (yumn eq 0 and yumx eq 0) then autoscl=1L
if (zumn eq 0 and zumx eq 0) then autoscl=1L
if (rumn eq 0 and rumx eq 0) then autoscl=1L
...
if(n_elements(press) eq 0) then press=2.10
 ...
 if(!d.name eq 'Z') then begin
   ...
   bkgrd=1  ; this "bkgrd" only changes the axis/labels colors, not the actual background....
 endif
...
;---------------------------------------------------------
; RCJ 03Jan2022  Portions moved from above
...
;         epoch=astrc.(ks).(epoch_index[ks]).dat(tind)
	 ...
	 ;
	 ; 15 s/c allowed so 15 symbols:
        ;symb=[' *',' x',' !9V!X',' !7D!X',' *',' x',' !9V!X',' !7D!X',' *',' x',' !9V!X',' !7D!X',' *',' x',' !9V!X']
        symb={num:[2,7,4,5,6,1,2,7,4,5,6,1,2,7,4,5], $
	      sym:[' !7*!X',' !4V!X',' !9V!X',' !7D!X',' !9B!X',' !7+!X', $ ; star,x,diamond,triangle,square,cross
	           ' !7*!X',' !4V!X',' !9V!X',' !7D!X',' !9B!X',' !7+!X', $
		   ' !7*!X',' !4V!X',' !9V!X',' !7D!X']}
	kks=symb.num[ks-1]
	
         ;draw_orbit,epoch,x,y,z,ks,xmp,rhomp,xbs,rhobs,doymark,hrmark,hrtick, $
         draw_orbit,epoch,x,y,z,kks,xmp,rhomp,xbs,rhobs,doymark,hrmark,hrtick, $
...
;help,s_len
        ...
        ;if(s_len ge 5) then all_sat3=s_prts[4]+' !9V!X' = diamond  ; !9B!X = square
        ;if(s_len ge 6) then all_sat4=s_prts[6]+' !7D!X' = triangle  ;  !7+!X or !3+!X = cross
...
;help,s_len
        if(s_len ge 1) then all_sat1=s_prts[0]+symb.sym[0]
        if(s_len ge 3) then all_sat2=s_prts[2]+symb.sym[1]
        if(s_len ge 5) then all_sat3=s_prts[4]+symb.sym[2]
        if(s_len ge 6) then all_sat4=s_prts[6]+symb.sym[3]
        if(s_len ge 9) then all_sat5=s_prts[8]+symb.sym[4]
        if(s_len ge 11) then all_sat6=s_prts[10]+symb.sym[5]
        if(s_len ge 13) then all_sat7=s_prts[12]+symb.sym[6]
        if(s_len ge 15) then all_sat8=s_prts[14]+symb.sym[7]
        if(s_len ge 17) then all_sat9=s_prts[16]+symb.sym[8]
        if(s_len ge 19) then all_sat10=s_prts[18]+symb.sym[9]
        if(s_len ge 21) then all_sat11=s_prts[20]+symb.sym[10]
        if(s_len ge 23) then all_sat12=s_prts[22]+symb.sym[11]
        if(s_len ge 25) then all_sat13=s_prts[24]+symb.sym[12]
        if(s_len ge 27) then all_sat14=s_prts[26]+symb.sym[13]
        if(s_len ge 29) then all_sat15=s_prts[28]+symb.sym[14]
...
;     xyouts,0.35,ytpos+.015,'symbols mark s/c at end of time range!C',charsize=1.2,/normal,color=fix(s_prts[1])
    ...
    if(s_len ge 25) then $
     xyouts,(0.71-xtpos),(ytpos-.033),all_sat13,charsize=chtsize1,/normal,color=fix(s_prts[25])
    if(s_len ge 27) then $
     xyouts,(0.86-xtpos),(ytpos-.033),all_sat14,charsize=chtsize1,/normal,color=fix(s_prts[27])
    if(s_len ge 29) then $
     xyouts,(1.01-xtpos),(ytpos-.033),all_sat15,charsize=chtsize1,/normal,color=fix(s_prts[29])

;$Author: tkovalic $
;$Date: 2022/06/29 15:43:46 $
;$Header: /home/cdaweb/dev/control/RCS/inventory.pro,v 1.37 2022/06/29 15:43:46 tkovalic Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.37 $
;
...
FUNCTION ingest_database,infile,DATASETS=DATASETS,mark_recent=mark_recent,DEBUG=DEBUG
;TJK 6/28/2022 - added the mark_recent keyword to indicate whether to
;                check the file dates to see if they were recently put
;                on the system and if so plot them "green" on the
;                inventory plot.  Possibly expensive so use carefully.

if keyword_set (mark_recent) then print, 'Checking each file for its system date - expensive'
  ...
  if (not further_check) then begin
	...
	while(strpos(a,'MISSION_GROUP') ne -1) do readf,1,a ; read next record in file
     ...
     for i=0L,ncdfs-1 do begin ; read each cdf line
       ...
       ; RCJ 17Jan2020   IBEX chokes here: b[1] is equal to b[2]
       ; So let's add 6mo or 1yr to the end time : 
       pts=strsplit(a,'/',/extract)
       w = where(pts eq 'ibex')
       if ( w[0] ne -1 and b[1] eq b[2] )then begin
          w1yr=strpos(b[0],'1yr')
	  if w1yr[0] ne -1 then begin
	     pts2=strsplit(b[2],'/',/extract)
	     b[2]=strtrim((pts2[0]+1),2)+'/'+pts2[1]+'/'+pts2[2]
	  endif
          w2yr=strpos(b[0],'2yr')
	  if w2yr[0] ne -1 then begin
	     pts2=strsplit(b[2],'/',/extract)
	     b[2]=strtrim((pts2[0]+2),2)+'/'+pts2[1]+'/'+pts2[2]
	  endif
          w3yr=strpos(b[0],'3yr')
	  if w3yr[0] ne -1 then begin
	     pts2=strsplit(b[2],'/',/extract)
	     b[2]=strtrim((pts2[0]+3),2)+'/'+pts2[1]+'/'+pts2[2]
	  endif
          w5yr=strpos(b[0],'5yr')
	  if w5yr[0] ne -1 then begin
	     pts2=strsplit(b[2],'/',/extract)
	     b[2]=strtrim((pts2[0]+5),2)+'/'+pts2[1]+'/'+pts2[2]
	  endif
          w11yr=strpos(b[0],'11yr')
	  if w11yr[0] ne -1 then begin
	     pts2=strsplit(b[2],'/',/extract)
	     b[2]=strtrim((pts2[0]+11),2)+'/'+pts2[1]+'/'+pts2[2]
	  endif
          w6mo=strpos(b[0],'6mo')
	  if w6mo[0] ne -1 then begin
	     pts2=strsplit(b[2],'/',/extract)
	     case pts2[1] of 
	      '01': begin
	        b[2]=pts2[0]+'/06/'+pts2[2]
		end
	      '02': begin
	        b[2]=pts2[0]+'/07/'+pts2[2]
		end
	      '06': begin
	        b[2]=pts2[0]+'/12/'+pts2[2]
		end
	      '10': begin
	        b[2]=strtrim((pts2[0]+1),2)+'/04/'+pts2[2]
	        end
	      '12': begin
	        b[2]=strtrim((pts2[0]+1),2)+'/06/'+pts2[2]
	        end
	     endcase 	
	  endif
          w10mp=strpos(b[0],'10mp')
	  if w10mp[0] ne -1 then begin
	     pts2=strsplit(b[2],'/',/extract)
	     ; RCJ 28Jul2021   these are just a copy of the cases above, I don't know
	     ;     what all the cases could be here.....
	     case pts2[1] of  
	      '01': begin
	        b[2]=pts2[0]+'/06/'+pts2[2]
		end
       ...
       if (b[1] ne b[2]) then begin
         ...
         cstops(i)  = b[2]
         recent(i) = 0
;TJK 6/28/2022 trying to speed up this code and we weren't using the
;"recent" information anyway, so commenting all of this out so
;it won't be checking the file dates for all data files.  So
;just set recent(i) to 0 (above)
         if keyword_set (mark_recent) then begin 
           ;TJK 8/22/2005 - new section to record whether file has recently been
;          put on the system or not - we want to color code this
;          on the inventory plot - this checks the file system
;          date for each file - pretty i/o intensive
           ...
           year = cdf_date[0]
           mm = cdf_date[1]
           ;need to improve this date comparison
           ;logic to make it useful, for now just hardcode some dates
           if (((year eq 2022) and (mm gt 1)) or (year ge 2023)) then begin
;             print, 'file ', b[0], 'put on system since 2022 Jan: ',cdf_date
             recent(i) = 1
           endif else recent(i) = 0
        endif ;if mark_recent keyword set
...
;---------------------------------------------------------------------------
 ...
 FUNCTION draw_inventory,a,TITLE=TITLE,GIF=GIF,PNG=PNG,DEBUG=DEBUG, START_TIME=start_time,$
...
for i=0L,ntags-1 do begin
  if (use(order(i)) eq 1) then begin
    ...
    ; RCJ 11Feb2020  Interesting idea if we want to bring it back one day.
    ;               For now we are just rotating 3 colors
    ...
    ;endif   
    ;for each file's start/end date determine the julian day values
    for j=0L, n_elements(b)-1 do begin
      ...
      e[j]=julday(parts[1],parts[2],parts[0]) ; e contains the julian start day
      ;
      parts=strsplit(c[j],/extract) ; will split on the blank space
      parts=fix(strsplit(parts[0],'/',/extract)) ; turns strings into integers
      f[j]=julday(parts[1],parts[2],parts[0]) ; f contains the julian end day
    ...
    ;if keyword_set(debug) then print, 'start and stop requested range ',bjul,ejul
    vals_in_range = 0
    ;TJK 6/16/2022 add a check for when both the requested start and end time are
    ;beyond the end of the available data for a given data set
    q=where(bjul gt e and  ejul gt f, n_found)
    if (q[0] ne -1 and n_found eq n_elements(f)) then begin
       ;comment this out as its too much
       ;info. even for debug, but need the if statement.
       ;if keyword_set(debug) then begin
       ;   print, 'BOTH requested start and end times are beyond the end of the available data'
       ;   print, 'SO do not want to reassign values as is done below'
       ;endif
    endif else begin
    ;compare the julian start and end day values w/ the requested
    ;begin/end julian dates and only keep the ones in range
      vals_in_range = 1
      q=where(e lt bjul)
   ...
   endelse
    ...
    save_color = color
;go through each begin date and set the bars array to a color for each
;day (within the requested start/stop range)
    num_starts = n_elements(e)
    for j=0L, n_elements(e)-1 do begin
      ...
      if (time(j) eq 1) then color = cgcolor('green') else color = cgcolor(save_color)
      ;TJK put in following check - 5/5/1999
      if (vals_in_range and (e(j)-bjul) le (f(j)-bjul)) then bars(index,(e(j)-bjul):(f(j)-bjul)) = color
      ;if index eq 128 then print,'bars: ',index,f(j),bjul,(f(j)-bjul)
   endfor ; end n_elements in e (e contains the start times of each file)
    ; increment the bars index
    index = index + 1
  endif
endfor ;end n_elements in b (b contains the start time of each file)
...
if (keyword_set(GIF) or keyword_set(PNG)) then begin
  if (keyword_set(GIF)) then myGIF=GIF 
  if (keyword_set(PNG)) then myPNG=PNG 
;  if keyword_set(debug) then begin
;     print,'DEBUG bar_names ', bar_names
;     print, 'DEBUG bars ', bars
;     print, 'DEBUG times ', times
;  endif

  s = bar_chart(bar_names,bars,times,TITLE=mytitle,GIF=myGIF,PNG=myPNG,xsize=1100,BIGPLOT=bigplot,$

;$Author: tkovalic $
;$Date: 2022/05/16 20:31:54 $
;$Header: /home/cdaweb/dev/control/RCS/plot_spectrogram.pro,v 1.132 2022/05/16 20:31:54 tkovalic Exp tkovalic $
;$Locker: tkovalic $
;$Revision: 1.132 $
;+------------------------------------------------------------------------
...
FUNCTION plot_spectrogram, Xvar, Yvar, Zvar, $
                          ...
                          CDAWEB=CDAWEB,GIF=GIF,PNG=PNG,NOCLIP=NOCLIP,$
...
; Adding documentation of the time sorting algorithm so future maintainers may
; have an easier life :).

; q is an array indexes of times that are monotonic.  
; TIA... it is actually times that are either monotonic or FILLVAL
; the loop exits when all times meet this criteria.  That is, all times are
; now monotonic.

; The loop works by checking for times which occur before their predacessors.
; These non-conforming times are eliminated on each pass, and then the new
; time array is rechecked.  Only when all times are conforming does the loop
; exit.

; Since data associates with times, corresponding data values are also
; eleminated when a time is removed.
...
if n_elements(times) ne 1 then begin
   ...
   while done eq 0 do begin
      ...
      ; Need to add line to check number of dimmensions in the data. If
      ; it is more than four than we can't process it.
      ; Ron Yurow (Apr 5, 2018)
      IF  N_ELEMENTS (dim) gt 4 THEN BEGIN
          PRINT, 'ERROR=Can not remove time discontinuities from data with more than 4 dimensions.' 
          RETURN, -1
      ENDIF 
...
;             stop;
       ...
       ntimes=ntimes[q]
       ; Is this statement even necessary? It doesn't look like it gets executed. RY
     ...
     ; Add section of code to handle data with 4 dimensions.  This section is anologous
     ; to the previous one, only it is designed to handle 4 dimensional data.  This is 
     ; required for MAVEN and may be needed for future missions.
     ; Ron Yurow (Apr 5, 2018)
     IF n_elements(dim) eq 4 THEN BEGIN ; 4D
       qq = WHERE (dim eq N_ELEMENTS (times), nqq)
       IF (nqq gt 0) THEN BEGIN
          IF  KEYWORD_SET (DEBUG) THEN BEGIN
             PRINT, 'data before trim ' & HELP, nthedata
          ...
          CASE qq[nqq-1] OF 
            ...
            2: BEGIN
	             nthedata=nthedata[*,*,q,*] 
	           END 
            3: BEGIN ;should always be this case (time is the last array element)
	             nthedata=nthedata[*,*,*,q] 
	           END 
         ENDCASE
       ENDIF ELSE PRINT, 'plot_spectrogram: cannot adjust the data array'
     ...
     ; Need to check to make sure there elmements are still left in the ntimes array. For 
     ; certain cases, it will remove all times without achieiving monoticity.
     ; Ron Yurow    (February 10, 2022)
     IF  (n_elements (ntimes) lt 2) THEN BEGIN
         PRINT,'ERROR=Epoch values are not monotonic and cannot be re-ordered.'
         PRINT,'STATUS=Data is unsuitable for spectrogram display, please select a different time range.'
         RETURN, -1
     ENDIF
...
;      nt=ntimes[1:*]-ntimes
;      q=where(nt le 0)
;      if q[0] eq -1 theplot_spectrogram.pron done=1
;   endwhile
;   if keyword_set(DEBUG) then print,'done w/ while loop in ',systime(1)-ttt,' seconds.'
;   times=ntimes
;   thedata=nthedata
...
;if ((b eq 0) OR (b gt 5plot_spectrogram.pro)) then begin  
  ...
  4   : begin ; #panels determined by dimensionality or by display type
           ...
           if (b[0] ne -1) then begin ; evaluate the display type
              dtype = examine_spectrogram_dt(Zvar.DISPLAY_TYPE, thedata=thedata, data_fillval=zvar.fillval, $
	               valid_minmax=[zvar.VALIDMIN, zvar.VALIDMAX], debug=debug) 
	      d=size(dtype)
              if (d[n_elements(d)-2] eq 8) then begin
                 use_zlist = 1 
                 ;TJK 5/10/2022 determine which dimension we're walking
                                ; and add elist assignment here -
                                ; valid values for zelist are
                                ; all 1's, 2's, or -2's
                                ; or 1,3,5,8,9  <- this is the zelist
                                ;                  we want to assign
                                ;                  to elist
                 uniq1 = uniq(dtype.zelist1) & dim1 = size(dtype.zelist1,/dim) 
                 uniq2 = uniq(dtype.zelist2) & dim2 = size(dtype.zelist2,/dim)
                 uniq3 = uniq(dtype.zelist3) & dim3 = size(dtype.zelist3,/dim)
                 if (n_elements(uniq1) eq dim1) then  elist = dtype.zelist1
                 if (n_elements(uniq2) eq dim2) then  elist = dtype.zelist2
                 if (n_elements(uniq3) eq dim3) then  elist = dtype.zelist3
                 if keyword_set(DEBUG) then print, 'debug determined elist = ', elist
...
if(igram eq 1) then begin
   ...
   ; Added the following section to handle PostScript output
   ; Ron Yurow (08/15/2018)
   IF  (!D.name eq 'PS') THEN BEGIN
        scale = (num_panels) * (PANEL_HEIGHT + 80) 
        sizewindow = [!d.X_SIZE, scale + !d.Y_SIZE]
        DEVICE, XSIZE = sizewindow [0] /1000., YSIZE = sizewindow [1] / 1000.
   ENDIF
endif
...
if (new_window eq 1) then begin
   if keyword_set(GIF) or keyword_set(PNG) then begin ; set default gif sizes
...
endelse
if(igram eq 1) then begin
   ; The follow lines set up the inition position of the ionogram plots.
   ; The PostScript device is handled separately.
   ; Ron Yurow (August 15, 2018)
   IF  (!D.name eq 'PS') THEN BEGIN
       ppos[0]= 3000  &  ppos[2]=!d.x_size-3000
       ppos[1]= (!d.y_size - 2500) - psize 
       ppos[3]= (!d.y_size - 2500)
   ENDIF ELSE BEGIN
...
print, 'debug beginning of z labeling section'
print, 'debug need to work on this section for the solar orbiter/maven 4-d data'
        ...
        if (use_zlist) then begin
           for l = 0, n_elements(dtype.lptrn)-1 do begin
              if (dtype.lptrn[l] eq 1) then begin ; get labling from labl_ptr_1
                 ...
                 if (a[0] ne -1) then begin
;TJK 5/12/2022      if (Zvar.(a[0])[0] ne '') then zlabel = Zvar.(a[0])
...
if (zmax eq zmin) then begin & zmax = zmax + 1. & zmin = zmin - 1. & endif
   ...
   if(igram eq 1) then begin
      ...
      ; The following lines adjusts vertical spacing between ionagrams.
      ; The PostScript device is handled separately.
      ; Ron Yurow (08/15/18)
      IF  (!D.name eq 'PS') THEN BEGIN
          ppos[3]=ppos[3]-(psize+1500) & ppos[1]=ppos[1]-(psize+1500) & clear_plot=1
      ENDIF ELSE BEGIN

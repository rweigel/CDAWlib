;$Author: johnson $
;$Date: 2018/02/08 15:10:57 $
;$Header: /home/cdaweb/dev/control/RCS/convert_log10.pro,v 1.7 2018/02/08 15:10:57 johnson Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.7 $
...
function convert_log10, astruct, orig_names, flip_vert=flip_vert
...
if (im_size[0] eq 3 or (im_size[0] eq 2 and no_images eq 0)) then begin 
  ...
  if (fwc gt 0 and nfwc gt 0) then begin ; if fill and good data found
    ...
    ;  RCJ 08Feb2018 taking alog10 of fillval will result in NaN.  Make these fillval gain.				 
    q=where(~ finite(new_image))
    new_image[q]=fillval				 
  endelse

  ;  RCJ 16Jan2020  Apply reverse on image if keyword flip_vert is set:
  if keyword_set(flip_vert) then begin
    flipimg=new_image ;  placeholder. images will be flipped
    if im_size[0] eq 3 then imgs=im_size[3] else imgs=1
    for j=0,imgs-1 do begin
     flipimg[*,*,j]= reverse(new_image[*,*,j])
    endfor
    new_image=flipimg
  endif
...
;  findex = tagindex('FUNCTION', vartags) ; find the FUNCTION index number
  ...
  if ((func_name eq 'convert_log10' or func_name eq 'convert_log10_flip_vert') and (strlowcase(next_0) eq strlowcase(c_0))) then begin

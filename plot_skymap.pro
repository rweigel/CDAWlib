;$Author: rcjohns1 $ 
;$Date: 2024/03/01 01:08:33 $
;$Header: /home/cdaweb/dev/control/RCS/plot_skymap.pro,v 1.37 2024/03/01 01:08:33 rcjohns1 Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.37 $
;+------------------------------------------------------------------------
; NAME: PLOT_SKYMAP
...
if keyword_set(DEBUG) then begin
	...
	;Added code to enable multiple line GIF file names for use by parse.ph 
        ;if (reportflag eq 1) then begin
        ;    printf,1,'I_GIF=',GIF & close,1
        ;endif 
	; RCJ 23Sep2019 Added if not keyword_set(movie) below
	if not keyword_set(movie) then begin
       ...
       atags=tag_names(astruct.(vnum))
        ...
        ;Added code to enable multiple line PNG file names for use by parse.ph
        ;if (reportflag eq 1) then begin

;$Author: rcjohns1 $ 
;$Date: 2024/03/01 01:09:42 $
;$Header: /home/cdaweb/dev/control/RCS/plot_images.pro,v 1.94 2024/03/01 01:09:42 rcjohns1 Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.94 $
;+------------------------------------------------------------------------
; NAME: PLOT_IMAGES
...
FUNCTION plot_images, astruct, vname, $
                      ...
                      CDAWEB=CDAWEB,DEBUG=DEBUG,COLORBAR=COLORBAR, PNG=PNG
...
if keyword_set(DEBUG) then begin
	bytmin = min(idat, max=bytmax,/nan)
...
if (b2[0] ne -1) then $
  psrce = strupcase(strmid(astruct.(vnum).Logical_file_id,0,9))
    ...
    GIF=strmid(GIF,0,(strpos(GIF,'.gif')-3))+gifn+'.gif'
      ...
      deviceopen,6,fileOutput=GIF,sizeWindow=[xs+xco,ys+30],COLORTAB=colortab
...
;          the Logical file id
    ...
    ;print, 'DATASET=',psrce
       ...
       deviceopen,7,fileOutput=PNG,sizeWindow=[xs+xco,ys+30],COLORTAB=colortab
...
;
  ...
  endelse
                                ...
                                ; Added section to limit the
                                ;        number of files that can be
                                ;        requested for PNG files (for
...
; end changes 12/11 RTB
  ...
  if keyword_set(GIF) then begin
    ...
    deviceopen,6,fileOutput=GIF,sizeWindow=[xs+xco,ys+40],COLORTAB=colortab

;      if (no_data_avail eq 0) then begin
       image_string = 'IMAGE='+GIF
;       if(reportflag eq 1) then printf,1,'IMAGE=',GIF
;       print,'IMAGE=',GIF
;TJK 7/31/2017 - ;Make this a formatted print, to force IDL to print 
;it all on one line (problem with string > 256 characters
...
;      endelse
     ...
     deviceopen,7,fileOutput=PNG,sizeWindow=[xs+xco,ys+40],COLORTAB=colortab
...
;    idat = bytscl(idat,min=idmin, max=idmax, top=!d.table_size-8)
    idat = bytscl(idat,min=idmin, max=idmax, top=!d.table_size-2)
;print, 'TJK DEBUG after bytescale',  min(idat), max(idat)
...
;4/9/2001
    ; Make sure that it really is UVI data we are working with.  Other data sets may
    ; have a data type that begin with 'H2' or 'H3'.
    ; Ron Yurow  (May 17, 2018)
    ;if ((d_type[0] eq 'H2') or (d_type[0] eq 'H3')) then begin
    ; tmp_img=congrid(idat2[*,*,j],xpimg,ypimg) 
    ;endif else begin
    IF  (descriptor[0] eq 'UVI') and ((d_type[0] eq 'H2') or (d_type[0] eq 'H3')) THEN BEGIN
           tmp_img=congrid(idat2[*,*,j],xpimg,ypimg) 
    ENDIF ELSE BEGIN

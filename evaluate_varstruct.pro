;$Author: tkovalic $
;$Date: 2024/03/19 15:58:04 $
;$Header: /home/cdaweb/dev/control/RCS/evaluate_varstruct.pro,v 1.82 2024/03/19 15:58:04 tkovalic Exp $
;$Locker:  $
;$Revision: 1.82 $
...
FUNCTION evaluate_varstruct, a, autoplot_nodepend=autoplot_nodepend
...
if ((timing_data eq 1)OR(plottable_data eq 1)) then begin
  print, 'DEBUG evaluate_varstruct: varname = ',a.VARNAME
  b = tagindex('DAT',atags)
  if (b[0] ne -1) then begin
    d = a.DAT & dsize=size(d)
   ;TJK 6/20/2023, need to exclude fill values from the timing data in
   ;order to get a meaningful min/max, so get the fillval
    f = tagindex('FILLVAL',atags)
    if (f[0] ne -1) then fillvalue = a.FILLVAL ;don't know what to see for the "else"...
    if (timing_data) then begin
      case 1 of
     	 ...
     	 eptt2000: begin
                   agood = where((d ne fillvalue)) ;TJK 6/21/2023 add check for epoch values ne fill
                   if agood[0] ne -1 then begin
     		     pstruct.btimett2000=min(d[agood])
     		     pstruct.etimett2000=max(d[agood])
                   endif
;     		   pstruct.btimett2000=min(a.DAT)
;     		   pstruct.etimett2000=max(a.DAT)
     		   end         
         else: begin
              ;TJK 3/19/2024 also compare with the value of year 1930 because checking for value 0.0
              ;isnt good enough
               cdf_epoch, year_1930, 1930,1,1,0,0,0,0,/compute
               ;print, 'year_1930', year_1930
               ;stop;
;TJK 5/13/99 add check for epoch values of 0.0 change to 1930 because we have at least one data set that has bad values that aren't 0.0.               
	       agood = where((d gt year_1930)) 
;	       agood = where((a.DAT gt 0.0)) ;TJK 5/13/99 add check for epoch values of 0.0
     	       if agood[0] ne -1 then begin
    ...
    if (b[0] ne -1 and a.handle ne 0) then begin
     ...
     ;order to get a meaningful min/max, so get the fillval
      f = tagindex('FILLVAL',atags)
      if (f[0] ne -1) then  fillvalue = a.FILLVAL else fillvalue=494949
...
;;TJK		   if (dn[0] ne ' ') then pstruct.npanels = n_elements(dn)
           ...
           ; Check for error from examine_spectrogram_dt
           ; Ron Yurow  (Feb 23, 2018)
           IF SIZE (e, /TYPE) ne 8 THEN RETURN, -1 
  ...
  'SPECTROGRAM': begin
           ...
           ; Check for error from examine_spectrogram_dt
           ; Ron Yurow  (Feb 23, 2018)
           IF SIZE (dtype, /TYPE) ne 8 THEN RETURN, -1 
                   n=dtype.npanels
		   ; RCJ 03/15/2013  
...
case pstruct.ptype of ; check for missing depends
 1   : begin
          if (d0 eq 0) then pstruct.ptype = 0
          if (d0 eq 0) then autoplot_nodepend = 1
       end	  
 2   : begin
          if ((d0 eq 0)OR(d1 eq 0)) then pstruct.ptype = 0
          if ((d0 eq 0)OR(d1 eq 0)) then autoplot_nodepend = 1
       end
 3   : begin
          if ((d0 eq 0)OR(d1 eq 0)OR(d2 eq 0)) then pstruct.ptype = 0
          if ((d0 eq 0)OR(d1 eq 0)OR(d2 eq 0)) then autoplot_nodepend = 1
       end
 4   : begin
          if ((d0 eq 0)OR(d1 eq 0)OR(d2 eq 0)) then pstruct.ptype = 0
          if ((d0 eq 0)OR(d1 eq 0)OR(d2 eq 0)) then autoplot_nodepend = 1

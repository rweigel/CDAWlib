;$Author: ryurow $
;$Date: 2024/01/30 20:54:02 $
;$Header: /home/cdaweb/dev/control/RCS/read_myCDF.pro,v 1.408 2024/01/30 20:54:02 ryurow Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.408 $
;
...
if (zflag eq 1) then begin ; read the z-variable
   ...
   if zvinfo.MAXREC eq -1  then begin  ; this means NO records have been written
      ;Removed "WARNING" label from meesage.
      ;Ron Yurow (Aug 15, 2022)
      ;if keyword_set(DEBUG) then print,'WARNING=',vname,' has ZERO records!'
      if keyword_set(DEBUG) then print, vname,' has ZERO records!'
...
;     if keyword_set(DEBUG) then print, '2 Took ',systime(1)-vtime, ' seconds to do cdf_varget for ',vname
   ...
   if rcount eq 0 then begin
      ...
      print, vname,' has ZERO records!' & return,0
...
;TJK 9/15/2023 change initialization from 0 to -1 otherwise, further down the code
;thought there was a virtual variable properly define, when in some
;cases, there wasn't
;virs_found = 0L ;TJK 11/28/2006 set a flag, so that if virtual variables are found in the master, 
                ;don't check in the data cdf.
virs_found = -1L ;TJK 11/28/2006 set a flag, so that if virtual variables are found in the master, 
                ;don't check in the data cdf.
...
;print, 'virs_found =', virs_found
      if (num_virs ge 0) then virs_found = 1L
      if keyword_set(DEBUG) then print, 'num_virs = ',num_virs, ' and virs_found', virs_found
...
;      if keyword_set(DEBUG) then print, 'Processing table variable, ',vnames[t]
 ...
 ;        if keyword_set(DEBUG) then print, 'found new variable, adding to table, ',vnames(t)
   ...
   ; RCJ 17May2024   Table.varname and table.equiv contain the var names
   ;               and var names corrected for bad characters but not in the
   ;               order they appear in the output structure.  This sequence
   ;               is dictated by vnames. So create a var origvnames containing
   ;               the original varnames, before 'bad character' correction.
   ;               We are doing this for write_mycdf.pro, so we can write the
   ;               original var names there. 
   qorig=where(strpos(vnames,'$') gt -1)
   if qorig[0] ne -1 then begin
     origvnames=''
     for vx=0,n_elements(vnames)-1 do begin
       q=where(table.equiv eq vnames[vx])
       origvnames=[origvnames,table.varname[q[0]]]
     endfor
     origvnames=origvnames[1:*]
   endif  
   ...
   if keyword_set(NODATASTRUCT) then begin
      ...
      if qorig[0] ne -1 then myvname = create_struct('origvname',origvnames[0])  ; create .cdftype structure
      ...
      if qorig[0] ne -1 then mytype = create_struct(mytype,myvname)
      ...
      mytype = create_struct(mytype,myvary)       ; append the structures - TJK added 8/1/2001
      if qorig[0] ne -1 then mytype = create_struct(mytype,myvname)
      mytype = create_struct(mytype,mysize)
      mydata = create_struct(mytype,mydata)       ; append the structures
      ;
      mydata = create_struct(metadata,mydata)     ; append the metadata
      burley = create_struct(vnames[0],mydata)    ; create initial structure
   ...
   for vx = 1,n_elements(vnames)-1 do begin ; retrieve and append
      ...
      if keyword_set(NODATASTRUCT) then begin
         ...
         if qorig[0] ne -1 then myvname = create_struct('origvname',origvnames[vx])  ; create .cdftype structure
         mysize = create_struct('idlsize',size(data)) ; create .idlsize structure
...
;      findex = tagindex('FUNCTION', vartags) ; find the FUNCTION index number
      ...
      if (findex[0] ne -1) then begin ;found a virtual value w/ a function definition
         ...
         IF  SIZE (burley, /TYPE) NE 8 THEN BEGIN
             ...
             ; Instead of returning -1 here (which is useless for CDAWeb) we are going to return the
             ; standard error structure consisting of STATUS, ERRROR and DATASET keywords.
             ; This structure is return at various other places in read_myCDF
             ; Ron Yurow (January 30, 2024)
             ; RETURN, -1
            d_set='DATASET='+strupcase(atmp.logical_source)
            v_err="ERROR=Virtual function processing failed for: " + STRING (vir_vars.name[i]) 
            v_stat='STATUS=Data not available'
            tmp_str=create_struct('DATASET',d_set,'ERROR',v_err,'STATUS',v_stat)
            return, tmp_str            
         ENDIF

;$author: $ 
;$Date: 2024/03/01 01:09:15 $
;$Header: /home/cdaweb/dev/control/RCS/plot_fluximages.pro,v 1.41 2024/03/01 01:09:15 rcjohns1 Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.41 $
;+------------------------------------------------------------------------
...
if COLORBAR then begin 
 ...
 !x.margin[1] = 14
    ...
    if keyword_set(GIF) then begin 
       ...
       temp_png = png + strtrim(string(frame-1),2)
    endif
...
print, 'asking for image size of ',win
    ...
    stat = plot_enaflux(edat[FRAME-1],idat, spin, polar, pos_dat[*,FRAME-1], $
	   ...
	   nocolorbar=nocolorbar, smooth=smooth, gif=temp_gif, png=temp_png, reverseorder=reverseorder)	
...
;open the final gif file
      if keyword_set(GIF) then begin
         ...
         if (png) then begin
            deviceopen,7,fileOutput=PNG,sizeWindow=[win[0]+xco,win[1]+35]
            if (reportflag eq 1) then begin
...
;      acolor = !d.n_colors ; pick color for contouring
  ...
  endelse
    ...
    deviceclose
	  ;TJK added - delete temporary png file
	  cmd = strarr(2)
	  cmd[0] = "rm"
	  cmd[1] = temp_png
	  spawn, cmd, /noshell
		  ...
		  itmp = idat
	    	  idat = fltarr(12,12,nimages)
                  if keyword_set(GIF) then begin
                     ...
                     for numpng = 0, nimages-1 do begin
                       idat[*,*,numpng] = itmp[18:29,*,numpng]
                    endfor
                  endif
		  spin = 96.0
		  polar = 8.0
...
;original euv settings /noborder, /nocircles, /nodipole, /nocolorbar, gif=temp_gif)	
   ...
   for numpng = 0, nimages-1 do begin
      win=make_array(2, /integer, value=tsize)
      temp_png = png + strtrim(string(numpng),2)

      stat = plot_enaflux(edat[numpng],idat[*,*,numpng], spin, polar, $
                          pos_dat[*,numpng], spin_dat[*,numpng], 1, wsize=win, DEBUG=DEBUG,$
                          noborder=noborder, nocircles=nocircles, nodipole=nodipole, $
                          nocolorbar=nocolorbar, smooth=smooth, png=temp_png, reverseorder=reverseorder)	

      if (stat ge 0) then begin 
         if (n_elements(temp_pngs) eq 0) then temp_pngs = temp_png else temp_pngs = [temp_pngs,temp_png]
      endif else begin
         print, 'Png not generated for png number ',numpng,' and date ',edat[numpng] 
      endelse
   endfor
endif
...
;all time records had a gif produced for them. TJK added on 7/5/2001
  ...
  if keyword_set(GIF) then nimages = n_elements(temp_gifs)
  if keyword_set(PNG) then nimages = n_elements(temp_pngs)
  ...
  if keyword_set(GIF) then begin
    deviceopen,6,fileOutput=GIF,sizeWindow=[xs+xco,ys+40]
     ...
     deviceopen,7,fileOutput=PNG,sizeWindow=[xs+xco,ys+40]
      if (no_data_avail eq 0) then begin
       if(reportflag eq 1) then printf,1,'IMAGE=',PNG
       print,'IMAGE=',PNG
      endif else begin
       if(reportflag eq 1) then printf,1,'I_PNG=',PNG
       print,'I_PNG=',PNG
      endelse
  endif else begin ; open the xwindow
...
;     print, 'now read small image out of gif file named ',temp_gifs(j),' and put in the final file called',gif
     ...
     if keyword_set(GIF) then read_gif,temp_gifs[j],a,r,g,b ; & tvlct,r,g,b  ; read the gif and load colors from it
     if keyword_set(PNG) then read_png,temp_pngs[j],a,r,g,b
...
;     if (((yyyymmdd ne prev_yyyymmdd) or (j eq 0)) and tsize gt 50 ) then begin
     ...
     cmd = strarr(2)
     cmd[0] = "rm"
     if keyword_set(GIF) then cmd[1] = temp_gifs[j]
     if keyword_set(PNG) then cmd[1] = temp_pngs[j]
     spawn, cmd, /noshell

;$Author: tkovalic $
;$Date: 2021/09/21 18:09:10 $
;$Header: /home/cdaweb/dev/control/RCS/plot_stack.pro,v 1.65 2021/09/21 18:09:10 tkovalic Exp tkovalic $
;$Locker: tkovalic $
;$Revision: 1.65 $
;+------------------------------------------------------------------------
; NAME: PLOT_STACK
...
use_labl = 0L ;by default do not use the labl_ptr_1 values on the color bar (use the actual values) 
...
;check to see if display_type is set to use labl_ptr_1 on the right
if (b[0] ne -1) then begin ; evaluate the display type
   ...
   ; check to see if the use_labl display_type argument is set, if so
   ; use the labl_ptr_1 attribute values below
     scn=where(strupcase(c) eq 'USE_LABL',sn)
     if (sn gt 0) then use_labl = 1L else use_labl = 0L
   endif
endif
...
if (keyword_set(DEBUG)) then print, 'after applied to data all_ymin/max = ',all_ymin, all_ymax
  ...
  if ((all_ymax eq all_ymin) and $
  (round(all_ymin,/L64) eq round(Yfillval,/L64))) then begin
      ylabel = ''
      a = tagindex('FIELDNAM',YTAGS)
      if (a[0] ne -1) then ylabel = Yvar.(a[0])
...
;        mydata = mydata[w] & mytimes = mytimes[w] & w=0
  ...
  if (a[0] ne -1) then ylabel = Yvar.(a[0])
;these are the labels for the z axis, when not using a colorbar
 ;don't want these for stack plots with colorbar
  alt_clabel = '' ; define default
  if (use_labl) then begin ; also don't want to use them by default as not all variables have appropriate labels
    a = tagindex('LABL_PTR_1',YTAGS)
    if (a[0] ne -1) then begin
;     if (Yvar.(a[0])[0] ne '') then ylabel = Yvar.(a[0])(elist(i))
      if (Yvar.(a[0])[0] ne '') then alt_clabel = Yvar.(a[0])(elist(i))
    endif
  endif
;print, 'ylabels coming from labl_ptr_1 ', ylabel
...
;use the value of color before its been bytscaled
  ...
  ;test to see if there's room for number labels vs. colorbar
    ...
    if (colorbar eq 0) then begin
      fmt='(F0.1)'
      ; RCJ 09Dec2019  This is by no means the definitive method to determine the label format,
      ;  just google it and you'll see all of the potential problems.  But it does solve the 
      ;  cases we have at the moment.
      tmp1=[tmpcolor*1.-(fix(tmpcolor*1.,type=3))]  ; type=3 is long type
      tmp2=[tmpcolor*10.-(fix(tmpcolor*10.,type=3))]
      tmp3=[tmpcolor*100.-(fix(tmpcolor*100.,type=3))] 
      q1=where(tmp1 ne 0.0)
      if q1[0] eq -1 then fmt='(F0.1)'
      if q1[0] ne -1 then begin
        q2=where(tmp2 ne 0.0)
        if q2[0] ne -1 then fmt='(F0.2)'
        if q2[0] ne -1 then begin
          q3=where(tmp3 ne 0.0)
          if q3[0] ne -1 then fmt='(F0.3)'
        endif  
      endif
      ;
      zidx = strtrim(string(tmpcolor[i],format=fmt),2)+' ' ;energy val, how many decimal places is determined above
      ;print, 'original numeric label ',zidx
      verticall = 1
      if (alt_clabel ne '') then begin
         zidx = alt_clabel      ; use the character labels if they exist
         verticall = 0
      endif

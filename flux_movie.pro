;$author: $ 
;$Date: 2024/03/01 01:05:04 $
;$Header: /home/cdaweb/dev/control/RCS/flux_movie.pro,v 1.34 2024/03/01 01:05:04 rcjohns1 Exp rcjohns1 $
;$Locker: rcjohns1 $
;$Revision: 1.34 $
;+------------------------------------------------------------------------
; NAME: FLUX_MOVIE
...
; Write out Movie filename, that will contain all the images
  if (no_data_avail eq 0) then begin
   if(reportflag eq 1) then printf,1,format='(a)','MGIF='+GIF
     print,format='(a)','MGIF='+GIF
  endif else begin
     if(reportflag eq 1) then printf,1,format='(a)','MGIF='+GIF ; "I_GIF"??
       print,format='(a)','MGIF='+GIF
  endelse

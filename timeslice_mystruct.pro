...
;    endif else rbegin = w[0]
    ...
    ; Need to check cdf type of tstart / stop here -- they need to be in epoch16.  
    ; After processing any variable, tstart/tstop will be in correct format, so we won't need to 
    ; repeat this operation
    ; Ron Yurow  (July 14, 2020)
     epoch_type  = SIZE (epoch, /TNAME)
     tstart_type = SIZE (tstart, /TNAME)
     tstop_type  = SIZE (tstart, /TNAME)

     IF  (epoch_type eq 'LONG64' && tstart_type eq 'DCOMPLEX' && tstop_type eq 'DCOMPLEX' ) THEN BEGIN ; TT2000 case

         CDF_EPOCH16, tstart, year, month, day, hour, min, sec, milli, micro, nano, trash, /BREAKDOWN_EPOCH
         CDF_TT2000,  tstart, year, month, day, hour, min, sec, milli, micro, nano, /COMPUTE_EPOCH

         CDF_EPOCH16, tstop, year, month, day, hour, min, sec, milli, micro, nano, trash, /BREAKDOWN_EPOCH
         CDF_TT2000,  tstop, year, month, day, hour, min, sec, milli, micro, nano, /COMPUTE_EPOCH

     ENDIF

     IF  (epoch_type eq 'DOUBLE' && tstart_type eq 'DCOMPLEX' && tstop_type eq 'DCOMPLEX' ) THEN BEGIN ; EPOCH case

         CDF_EPOCH16, tstart, year, month, day, hour, min, sec, milli, trash1, trash2, trash3, /BREAKDOWN_EPOCH
         CDF_EPOCH,  tstart, year, month, day, hour, min, sec, milli, /COMPUTE_EPOCH

         CDF_EPOCH16, tstop, year, month, day, hour, min, sec, milli, trash1, trash2, trash3, /BREAKDOWN_EPOCH
         CDF_EPOCH,  tstop, year, month, day, hour, min, sec, milli, /COMPUTE_EPOCH
...
;    endif else rend = w(n_elements(w)-1)
    ...
    ; This line doesn't actually work if HANDLE isn't the last tag in the
    ; structure.  Replaced with a more general method.  
    ; Ron Yurow (Aug 28, 2020)
    ; if (vtags[nvtags-1] eq 'HANDLE') then begin
    sink = WHERE (STRUPCASE (vtags) eq "HANDLE", found)
    ...
    for j=0,nbtags-1 do begin
      ...
      if ti ne -1 then begin
        if b.(j).DEPEND_0 eq b.(i).VARNAME then begin
          ; Check for NRVs.  NRVs probably should not have a DEPEND_0, but
          ; some do.  If we find one then just skip to the next iteration.
          ; Ron Yurow (September 1, 2020)
          sink = WHERE (STRUPCASE (vtags) eq "CDFRECVARY", found)
          IF found gt 0 && STRUPCASE (b.(j).CDFRECVARY) eq "NOVARY" THEN CONTINUE

...
FORMAT10 ='(/1X,"**** RECALC WARNS: YEAR IS OUT OF INTERVAL 1965-2025. CALCULATIONS WILL BE DONE FOR IYEAR=",I4)'
...
;C
      ...
      IF(IY NE IYR) AND (IPR EQ 0)THEN PRINT,FORMAT=FORMAT10,IY
...
;TJK added use of FLOAT	   F2=(IY+IDAY/365D0-1995.)/5.
	...
	ENDIF ELSE IF (IY LT 2020) THEN BEGIN		;!2015-2020
...
;
	ENDIF ELSE BEGIN				;!2020-2025
;TJK added use of float and divide by 366 instead of 365
;	   DT=IY+IDAY/365D0-2010.
           DT=FLOAT(IY)+FLOAT(IDAY-1)/365.25-2020.
	   G10=29404.8-5.7*DT      ;! HERE G10 HAS OPPOSITE SIGN TO THAT IN IGRF TABLES
	   G11=-1450.9+7.4*DT  ; multiplying by DT are elements from igrf??s.dat: -G10 (yes,negative), 
	   H11=4652.5-25.9*DT    ; G11 and H11.
	ENDELSE
;C

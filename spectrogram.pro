...
;
;-
; Need to pass errors up the stack
ON_ERROR, 2
...
status = 0L
; The following code was reimplemented in order to provide a conveneint method
; for creating a 'no data' plot in response to a variety of error conditions.
; The 'no data' plot is a blank plotting area with the message: 'No good values
; to display for <variable>'.  Errors are thrown by using the MESSAGE procedure 
; when a condition is detected that makes it impossible to create a spectragram.
; Since runtime errors will also be caught, only user generated errors (-5) will
; cause a 'no data' plot to be created.  All other errors will be re-issued.
...
;endif
if doStatus then begin
   ; Reset the defualt error message
   MESSAGE, /RESET   
 
   ; Install an error handler to create a 'no data' plot in response to
   ; user generated errors.  Other error messages are not handled.
   CATCH, error_status
   IF (error_status ne 0) THEN BEGIN
      CATCH, /CANCEL 
      IF error_status eq -5 THEN BEGIN
         ...
         xyouts,(extra.position[2]-extra.position[0])/2,$
	             ...
	             this_msg+parts[0],device=extra.device
          
         status = -1L
         RETURN
      ENDIF ELSE BEGIN ; Any other error.
...
if (Zsize[0] ne 2) then begin
   ...
   ; Consolidated MESSAGE procedure calls since errors are now handled w/ CATCH.
   ; Ron Yurow (April 19, 2018)
   ;if doStatus then begin
